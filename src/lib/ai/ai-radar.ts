...
function needsResolve(role: AiRadarSource['radarRole'], title: string, text: string): boolean {
  ...
  const hay = `${title}\n${text}`;
  return /\b(model|capability|safety|cyber|robot|breakthrough|pause|deploy|autonom|llm|gpt|claude|gemini)\b/i.test(
    hay,
  );
}
...
export function resolveToPrimary(
  ...
  if (tokens.size < 2) return { resolved: false };
  ...
  for (const p of primaryPool) {
    ...
    // Also boost shared rare tokens from text (astra, cyber, preparedness…)
    const rare =
      candidate.title.match(
        /\b(astra|preparedness|cyber|zero-?day|weathernext|lyria|gemini robotics)\b/gi,
      ) || [];
    for (const r of rare) {
      if (new RegExp(r, 'i').test(p.title + p.text + p.url)) overlap += 2;
    }
...
export async function collectAiRadarCandidates(opts?: {
  ...
  for (const { item, src } of rawItems) {
    const tk = topicKey(item.title);
    if (tk && seen.has(tk)) continue;
    if (tk) seen.add(tk);

    const { priority, signals } = classifyAiEvent(item.title, item.text || '');
    if (priority === 'drop') continue;

    const need = needsResolve(src.radarRole, item.title, item.text || '');
    ...
    if (primaryResolved) {
      ...
      const resolved = resolveToPrimary(item, primaryPool);
