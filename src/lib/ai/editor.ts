...
const EDITORIAL_DEPTH = [
  ...
  'ОБЪЁМ: норма 180–300 слов. 120–180 допустимо ТОЛЬКО если история простая и полностью раскрыта.',
...
const EDITOR_SYSTEM_PROMPT = [
  'Ты спокойный компетентный редактор SmartProto — не блогер, не продавец, не карточка товара, не переводчик пресс-релиза.',
  ...
  'Тон: EDITORIAL REVIEW / notice — самостоятельный обзор вокруг самого яркого честного факта;',
...
export async function writeDraft(articleData: object, reviewData: object): Promise<DraftResult> {
  ...
  const formatInstructions =
    format === 'news'
      ? [
          'ФОРМАТ: новость-обзор (SHORT REVIEW). Норма 180–260 слов; 120–180 только если история совсем простая.',
      ...
      : [
          ...
          '120–180 слов допустимо только если история простая и полностью раскрыта — иначе пиши 180–300.',
