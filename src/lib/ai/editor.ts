...
const EDITOR_SYSTEM_PROMPT = [
  ...
  '- хук под КОНКРЕТНЫЙ продукт: любопытство / выгода / недоверие / практичный вин / TikTok-share energy;',
  ...
  '- НИКОГДА не пиши слова/фразы: дожили / дожили до времени / вчера казалось фантастикой / вчера фантастика — сегодня…;',
  '- эти штампы запрещены навсегда в title и text — ни как хук, ни в середине, ни в финале;',
  '- каждый текст — свой хук и акцент под ЭТОТ товар (не универсальная фраза «вау, будущее наступило»).',
  'Синонимы и углы хука (выбирай разное каждый раз): любопытство («подождите…»), выгода («это экономит X»),',
  'недоверие («звучит как реклама, но…»), практичный вин («наконец можно не…»), share-energy («скинь другу, который…»),',
  'сравнение («это не обычный X, а…»), сценарий дня, неожиданный факт из спеки.',
  '',
    ...
    messages: [
      { role: 'system', content: EDITOR_SYSTEM_PROMPT },
      {
        ...
        content: [
          ...
          'Структура: 1) УНИКАЛЬНЫЙ хук под этот продукт (curiosity / benefit / disbelief / practical win / TikTok-share);',
          'НИКОГДА не используй: дожили / дожили до времени / вчера казалось фантастикой — ни разу, ни как дефолт;',
          '2) что это и зачем в быту/работе; 3) 2–4 реальные фичи с эмоцией;',
          '4) цена/статус/где купить если есть; 5) честное ограничение если есть.',
          'Нет данных — так и скажи, не выдумывай.',
          'Финал — тоже разный: вопрос другу, практичный вывод, лёгкий doubt, share-punch — не одна формула.',
