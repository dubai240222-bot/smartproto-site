...
const REVIEW_SYSTEM_PROMPT_GADGET = [
  ...
  'EDITORIAL DNA v1.1 (SP-A-068R1): HUMAN WALL → REAL TECHNOLOGY → OPENED DOOR → PROOF. Спроси: какую стену ломает и какую дверь открывает обычному человеку?',
  'PASS сильные двери (самому / один≈команда / меньше капитал / без посредника / pro→home / экономия / новая способность / assistive independence).',
  'RESEARCH с доказанной человеческой возможностью — не REJECT только из-за статуса; статус ограничивает обещание.',
  'REJECT или слабо: specs/+N%/Hz/megapixels; funding/partnerships; factory robot без human door / +15% cycle; humanoid demo без пользы; дорогая ниша если обычная альтернатива разумнее.',
  ...
  'keyAspects — 3 коротких пункта: стена / дверь / proof-status (RU или EN, не китайский; не про цену).',
