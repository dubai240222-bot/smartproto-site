...
/** Stock openers / endings / hype that must never ship. */
const STALE_PHRASE_RE =
  /дожил(?:и|а|о)?(?:\s+до\s+времени)?|вчера казалось фантастикой|вчера фантастика|представлена инновационн|представляем\s+\w|ребята[,!]?\s|guys|look at this|просто\s+вау|\bвау\b|wow[!]?|скинь\s+другу/i;

/** Soft monotony / playful cues — ban TikTok-style openers. */
const MONOTONE_OPENER_RE =
  /^(?:вау[,!]?\s+|wow[,!]?\s+)?(?:ребята[,!]?\s+|guys[,!]?\s+|подождите[.!]?\s+|смотрите[,:]?\s+|смотри[,:]?\s+|посмотрите[,:]?\s+|look at this[,!]?\s+|хотите\s+|сложно\s+угадать|представьте[:!]?\s+|это\s+же\s+)/i;

/** Pathos / hype slang — chief editor removes these. */
const MELODRAMA_RE =
  /гениально[!]?|просто\s+вау|\bвау\b|wow[!]?|guys|look at this|вы\s+не\s+поверите|это\s+чудо|огонь[!]|обалдеть|невероятн\w*\s+вау|офигенн|скинь\s+другу|не\s+поверите/i;
...
async function polishArticle(
    ...
    messages: [
      {
        role: 'system',
        content: [
          'Ты спокойный компетентный editor-in-chief SmartProto — финальный редактор корпуса карточек.',
          'Задача: убрать патос, игривость и хайп; сделать ясное взрослое product-объяснение.',
          'ГОЛОС АВТОРА: только мужской. ЗАПРЕЩЕНО женское 1-е лицо: «я пришла/увидела/поняла/решила»,',
          '«я готова/рада/уверена/была», «включила», «я как женщина», формы на -а/-ла/-лась.',
          'Тон: ясный, конкретный, без давления купить ASAP — через пользу и факты.',
          'НЕ патос, НЕ игривость, НЕ «вау/ребята/guys/look at this», НЕ эмодзи, НЕ TikTok-сленг.',
          'ЖЁСТКО: content 150–200 слов (считай слова; короче 150 — провал).',
          'СОХРАНЯЙ факты: цены, спеки, бренды, buy/preorder — только из исходника. НЕ выдумывай.',
          'ЗАПРЕЩЕНО: клише «будущее уже здесь» / «вчера это казалось невозможным»; «представлена инновационная…».',
          'Без «вау/wow/guys/ребята/смотрите/look at this» и пустых восклицаний.',
          'Хук через пользу / сценарий / сравнение / факт — без восклицаний.',
          'Не начинай с «Ребята…», «Вау…», «Guys…», «Подождите…», «Смотри…», «Представьте…».',
          'Структура: продукт → как работает → цена/доступность → ограничения / неизвестное.',
          'Title: продукт + польза/факт, без «!»; tags — без выдуманных брендов/людей.',
      ...
      },
      {
        ...
        content: [
          ...
          'content: ОБЯЗАТЕЛЬНО 150–200 слов, русский, спокойное product-объяснение, мужской голос, уникальный opener.',
          'Убери патос/игривость/хайп и женские формы 1-го лица. Разверни: хук → что это → отличия → цена/статус → ограничения.',
          'summary: 1–2 предложения из нового текста (до ~200 символов), без штампов, патоса и сленга.',
          'title: можно оставить или слегка улучшить (до 90 символов), без хайпа.',
  ...
  if (STALE_PHRASE_RE.test(blob) || MELODRAMA_RE.test(blob)) {
    throw new Error(`Inspect-polish still contains stale/hype phrases for ${article.slug}`);
  }
