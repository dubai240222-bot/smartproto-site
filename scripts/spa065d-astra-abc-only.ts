...
async function main() {
  ...
  async function s(title: string, text: string) {
    const r = await scoutArticle(title, text, 'ai_radar');
    return { score: r.score, reason: (r.reason || '').slice(0, 160), pass: r.score >= 70 };
  }
  const A = await s(ASTRA_PRIMARY.title, ASTRA_PRIMARY.text);
  console.log('A', A.score, A.reason);
  const B = await s(ASTRA_VERGE.title, ASTRA_VERGE.text);
  console.log('B', B.score, B.reason);
  const C = await s(ASTRA_PRIMARY.title, ev.summaryForScout);
  console.log('C', C.score, C.reason);
  ...
  prev.controlSet = prev.controlSet.map((r: { key: string }) =>
    r.key === 'astra'
      ? {
          ...r,
          rawScore: B.score,
